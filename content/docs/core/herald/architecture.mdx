...
- **Auth & tenancy**: an `Authorization: Bearer <key>` is hashed and matched to a tenant; routes decide whether a tenant is required. The same bearer key authenticates both the REST routes and the GraphQL endpoint at `/graphql`.
