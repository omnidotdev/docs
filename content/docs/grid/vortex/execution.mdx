...
Organizations can register their own Temporal cluster via the `workflow_executor_config` table. The executor field accepts a custom slug (not one of the built-in values) and the API looks up the encrypted connection config for the matching organization and slug.
...
The API resolves the slug to a stored connection config containing address, namespace, and TLS settings, then starts the workflow on the custom cluster. If no matching config is found, the API returns a `400` error.
