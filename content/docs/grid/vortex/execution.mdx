...
Vortex workflows are executed by the **vortex-worker** service. The API dispatches each run to one of four pluggable backends (Hatchet, Temporal, Local, or SpinKube), then the worker processes the DSL step-by-step.
...
| **SpinKube** | `"spinkube"` | WASM-based execution via SpinKube on Kubernetes |
...
Requires `TEMPORAL_ADDRESS` to be configured. In production, use `temporalio/server` with schema migrations run separately.
...
WASM-based execution via [SpinKube](https://spinkube.dev) on Kubernetes. Workflows are compiled to WebAssembly components and executed as Spin applications, providing lightweight sandboxed execution with fast cold starts.
...
Requires a Kubernetes cluster with the SpinKube operator installed. The adapter handles packaging DSL workflows as Spin apps and managing their lifecycle on the cluster.
...
1. Resolves the integration connector (e.g., `slack`)
2. Decrypts stored credentials for the organization
3. Builds the connector execution context
4. Executes the action with retry support
5. Maps outputs to workflow variables

## Integrations

Vortex ships with 600+ built-in integrations powered by the ActivePieces connector ecosystem. Each integration provides actions and triggers that can be used in workflow steps.

### Integration mapping

Integration IDs map to connector packages:

| Integration | Example operations |
|---|---|
| `slack` | `sendMessage`, `createChannel` |
| `github` | `createIssue`, `addComment` |
| `stripe` | `charge`, `createCustomer` |
| `google-sheets` | `appendRow`, `readRange` |

Integrations are dynamically loaded at runtime based on the integration ID in the step definition. Browse all available integrations on the [Integrations page](https://vortex.omni.dev/integrations) in the dashboard.
...
Built-in native plugins ship alongside WASM support, covering the same capabilities as external WASM plugins but implemented in TypeScript for zero cold-start overhead. Plugin access is gated to Pro and above plans.
...
External trigger adapters can be loaded from `VORTEX_TRIGGER_PLUGINS_DIR`, where each file exports a `createAdapter(config)` factory function.
