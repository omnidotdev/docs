...
| **Iggy** | 8090 (TCP) | Event streaming (OmniEvent bus) |
...
| `ENCRYPTION_KEY` | Yes | - | Base64-encoded 32-byte key for encrypting stored integration credentials (required when storing integration credentials) |
| `HATCHET_CLIENT_TOKEN` | Yes* | - | Token for the Hatchet workflow engine (required when using Hatchet executor backend) |
| `INTERNAL_API_SECRET` | Yes | - | Shared secret for internal service-to-service calls (generate with `openssl rand -hex 16`) |
...
| `AUTHZ_API_URL` | No | - | Warden AuthZ API URL (enables AuthZ when set) |
| `STRIPE_API_KEY` | No | - | Stripe secret key |
| `STRIPE_WEBHOOK_SECRET` | No | - | Stripe webhook signing secret |
| `GRAPHQL_MAX_COMPLEXITY_COST` | No | `5000` | Maximum GraphQL query cost (the `.env.local.template` suggests `2000`, but the code default is `5000`) |
...
|---|---|---|---|
| `DATABASE_URL` | Yes | - | PostgreSQL connection string |
| `HATCHET_CLIENT_TOKEN` | Yes* | - | Hatchet workflow engine token (required when using Hatchet executor backend) |
