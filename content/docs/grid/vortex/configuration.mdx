...
| `AETHER_BASE_URL` | No | — | Aether billing service URL |
| `STRIPE_API_KEY` | No | — | Stripe secret key |
| `STRIPE_WEBHOOK_SECRET` | No | — | Stripe webhook signing secret |
| `GRAPHQL_COMPLEXITY_MAX_COST` | No | `2000` | Maximum GraphQL query cost |
...
| `VITE_API_URL` | No | — | Vortex API base URL |
...
The API enforces a default rate limit of **100 requests per minute per IP**. GraphQL queries are cost-limited to a configurable maximum (`GRAPHQL_COMPLEXITY_MAX_COST`, default 2000).
