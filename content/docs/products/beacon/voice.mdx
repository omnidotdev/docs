...
2. **Speech-to-text** — Your spoken question is transcribed via [Synapse](/products/synapse)
3. **Text-to-speech** — The assistant's response is spoken aloud via [Synapse](/products/synapse)
...
Voice requires a running [Synapse](/products/synapse) instance with at least one STT and one TTS provider configured. The simplest setup is an OpenAI key, which covers both:
...
Configure providers in your Synapse config file. See the [Synapse docs](/products/synapse) for details.
...
Each persona has its own wake words defined in its [persona.json](/products/persona-json):
...
Edit your persona's `persona.json` to change or add wake words. See [Personas](/products/beacon/personas) for details on persona customization.
