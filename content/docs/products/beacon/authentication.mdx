...
Generate a password hash using an external tool:
...
Set the hash via environment variable or config:

```bash
BEACON_AUTH_PASSWORD_HASH="$argon2id$v=19$m=65536,t=3,p=4$..."
```
...
New devices must complete the pairing flow before they can authenticate. See [Device Identity](/products/beacon/device-identity) for details.
...
```bash
# Token mode
curl -H "Authorization: Bearer your-token" http://gateway:18789/api/...

# Device mode
curl -H "X-Device-Id: ..." -H "X-Signature: ..." http://gateway:18789/api/...
```
