...
- Checklists (break the work into trackable sub-items)
- Comments (for discussion)

### Checklists

Add one or more **checklists** to a task to break it into smaller steps. Each checklist has a name and a set of items you tick off as you go, and its progress (for example, 3/5) shows on the task card so you can see how far along a task is without opening it.

When a checklist item grows into work of its own, **convert it into a task**: the item becomes a new task in the project's first column and links back to the task it came from, so nothing gets lost.
...
Any project can be made **public**, giving anyone with the link a read-only view of the board — no sign-in required.

Projects are **private by default**. Only workspace members with the Admin or Owner role can toggle visibility in **Project Settings → Visibility**.

When a project is public:

- A shareable link is shown in settings (`/b/{workspace}/{project}`)
- A "Public" badge appears on the project header so members know the board is externally visible
- The public view is read-only: no drag-and-drop, no editing, no filters

When a project is private:

- Only authenticated workspace members can access it
- Visiting the public link returns a 404
