...
Synapse can be self-hosted from source, via Docker, or with prebuilt binaries. See the [Self-Hosting guide](/products/synapse/self-hosting) for installation options.
...
Synapse listens on the configured address (default `0.0.0.0:3000`). In Docker Compose, host port 6000 maps to container port 3000.
...
- [Configuration](/products/synapse/configuration) -- Full TOML reference with rate limits, failover, and TLS
- [Routing](/products/synapse/routing) -- Intelligent model selection strategies
- [Providers](/products/synapse/providers) -- All supported providers across every modality
- [API Reference](/products/synapse/api) -- Complete endpoint documentation with examples
