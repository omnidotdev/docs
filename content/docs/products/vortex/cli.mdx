...
<Callout type="warn">
The Vortex CLI is currently in development and not yet published. The commands documented below reflect the planned interface and may change before release.
</Callout>
...
Deploy workflows to the Vortex API.

```bash
vortex deploy
```

Validates all workflows, then pushes each to the Vortex API (equivalent to `vortex push`).
...
Reads `manifest.json` and `./plugin.wasm` (default path), then uploads via `POST /api/v1/plugins/upload`. Returns the plugin ID on success.
