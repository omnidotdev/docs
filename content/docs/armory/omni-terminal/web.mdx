...
The web terminal is built into the `omni-terminal` binary via the `serve` subcommand. The WASM frontend is embedded at compile time — no separate build step is needed when using a prebuilt binary.

### Quick Start

```bash
# if installed via package manager (Homebrew, AUR, etc.)
omni-terminal serve
...
omni-terminal serve --host 0.0.0.0 --port 8080

# use custom TLS certificates
omni-terminal serve --tls-cert /path/to/cert.pem --tls-key /path/to/key.pem

# plain HTTP (no TLS)
omni-terminal serve --no-tls
```
...
```bash
# build the WASM frontend (required before compiling the binary)
just wasm-build
